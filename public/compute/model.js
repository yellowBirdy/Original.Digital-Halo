...
function trackersFromVisit ({visit}) {
	...
	visit.thirdPTrackers && Object.entries(visit.thirdPTrackers).forEach(([tracker, company]) => {
		...
		companies.push(company[0]);    console.log(`tracker: ${tracker}, company ${company[0]}`)
	});

	return {companies, trackers};
}

function updateTrackerCompanyLists ({trackers, companies}) {

	var tL = deserialize(window.localStorage.getItem('trackerList')) || [];
	var uTl = [...new Set(tL.concat(trackers))];
	window.localStorage.setItem('trackerList', serialize(uTl));


	var cL = deserialize(window.localStorage.getItem('companyList')) || [];
	var uCl = [...new Set(cL.concat(companies))];
	window.localStorage.setItem('companyList', serialize(uCl));
...
function addAllNewVisitsToProfile ({profile, visits, model}) {
	...
	visits.forEach(visit => {
		...
		const {trackers, companies} = trackersFromVisit({visit});  console.log(`andVtoP ${trackers.join(',')} ${companies.join(",")}`)
		updateTrackerCompanyLists({trackers, companies});
...
function trackerCountsFromHistoryWithTrackers (priorCounts, historyWithTrackers) {  //  cookie Viz feed
	const trackerCounts = copy(priorCounts) || [];      //  Who's seen how much , via which trackers   -
	/*{ count: 11453,
	 //	name: 'Google',
	 //	children:
